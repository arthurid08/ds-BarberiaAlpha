...
## CAPÍTULO 1: Visão do Produto & Requisitos

Sistema de Agendamento - Barbearia Alpha

## Este repositório contém a aplicação web desenvolvida em Flask para gerenciamento de agendamentos da Barbearia Alpha.
...
Otimização do fluxo de agendamentos e atendimento da barbearia, eliminando a sobreposição de horários (conflitos na agenda) e automatizando a confirmação de serviços entre clientes e barbeiros sem dependência de atendimento manual contínuo.
...
## CAPÍTULO 2: Design e Arquitetura do Sistema
...
[Diagrama Arquitetural Completo](api.png)
...
1. Fluxo Síncrono (API REST)

Funcionalidade: Validação de Autenticação do Administrador (/login).
Mecanismo: Chamada HTTP/REST Síncrona.
Descrição do Fluxo:
O usuário preenche as credenciais no formulário web e envia uma requisição POST para a rota /login.
O API Gateway direciona a chamada para o backend Flask, que consulta de imediato o banco de dados via SQLAlchemy para checar o usuário e a senha criptografada.
A conexão permanece aberta (bloqueante) durante o ciclo da requisição.
O servidor devolve uma resposta HTTP síncrona enviando o cookie de sessão autenticada ou um alerta de erro de validação na tela.

2. Fluxo Assíncrono (Filas / Eventos)

Funcionalidade: Disparo de Notificação de Confirmação para o Evento "Quando o agendamento é realizado".
Mecanismo: Arquitetura Orientada a Eventos / Mensageria (Celery com RabbitMQ/Redis).
Descrição do Fluxo:
O cliente submete o agendamento enviando os dados para a rota /criar.
O backend confirma a disponibilidade, grava o registro no banco SQLite e altera o status do horário para "Ocupado".
Imediatamente após salvar, o sistema publica uma mensagem no barramento referente ao evento contendo os detalhes do serviço, do cliente e do barbeiro.
O cliente recebe a confirmação na tela sem precisar aguardar o envio de e-mail ou mensagens externas.
Em segundo plano, um Worker assíncrono consome o evento na fila e envia as notificações de confirmação (via e-mail ou WhatsApp API) para o cliente e para o barbeiro de forma totalmente desacoplada.
